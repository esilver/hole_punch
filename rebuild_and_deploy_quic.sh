...
echo "--- Starting QUIC Version Rebuild and Redeployment Process ---"

# === 1. Delete existing services AND build new images concurrently ===
echo "Step 1: Deleting services and building new QUIC images concurrently (with timeouts to avoid hanging)…"
...
echo "Building and pushing QUIC Worker service image with Docker Buildx…"
run_with_timeout ${BUILD_TIMEOUT_SEC}s docker buildx build --platform "${BUILD_PLATFORM}" \
  -t "${REGION}-docker.pkg.dev/${PROJECT_ID}/${AR_WORKER_REPO_NAME}/${WORKER_SERVICE_NAME}-quic:${WORKER_IMAGE_TAG}" \
  -f Dockerfile.worker.quic . --push & worker_build_pid=$!
...
wait_and_report $worker_build_pid "Worker QUIC image build"
...
echo "Step 2: Deploying Rendezvous and QUIC Worker services concurrently…"
...
wait_and_report $worker_deploy_pid "QUIC Worker service deployment"

echo "Both services deployed (or attempted) concurrently."

echo "--- QUIC Version Rebuild and Redeployment Process Completed ---"
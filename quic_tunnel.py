        ...
        try:
            while True:
                ...
                async with self._quic_state_lock:
                    ...
                    print(
                        f"Worker '{self.worker_id}': aioquic.get_timer -> {current_timeout}"
                    )
                ...
                if now_val - self._last_ping_time > 5.0:
                    async with self._quic_state_lock:
                        try:
                            print(
                                f"Worker '{self.worker_id}': QUIC sending PING uid={self._ping_uid_counter} to {self.peer_addr}"
                            )
                            ...
                            print(
                                f"Worker '{self.worker_id}': Next ping uid will be {self._ping_uid_counter}"
                            )
                        except Exception as e_ping:
                            print(f"Worker '{self.worker_id}': QUIC ping failed: {e_ping}")
                    self._last_ping_time = now_val
                    print(
                        f"Worker '{self.worker_id}': Ping timer reset at {now_val:.3f}"
                    )
                ...
                print(
                    f"Worker '{self.worker_id}': Timer loop sleeping for {delay:.3f}s"
                )
                await asyncio.sleep(delay)
                ...
                async with self._quic_state_lock:
                    try:
                        ...
                        handle_time_start = time.monotonic()
                        self.quic_connection.handle_timer(now=time.time())
                        print(
                            f"Worker '{self.worker_id}': handle_timer completed in {time.monotonic() - handle_time_start:.4f}s"
                        )
                        self._process_quic_events()
                        self._transmit_pending_udp()
